...
class RNNEnc(en.Module):
    def __init__(self, v, e, h):
        super().__init__()
        self.emb = en.Embedding(v, e, padding_idx=0)
        self.rnn = en.RNN(e, h, num_layers=1, batch_first=True, bidirectional=True)
        self.red = en.Linear(h * 2, h)

    def forward(self, x):
        e = self.emb(x)
        out, h = self.rnn(e)
        hcat = en.cat([h[0], h[1]], dim=1)
        h0 = en.tanh(self.red(hcat)).unsqueeze(0)
        return out, h0


class RNNDec(en.Module):
    def __init__(self, v, e, eh, dh):
        super().__init__()
        self.emb = en.Embedding(v, e, padding_idx=0)
        self.rnn = en.RNN(e, dh, num_layers=1, batch_first=True)
...
class RNNSeq(BaseModel, en.Module):
    def __init__(self, cfg, vocab):
        en.Module.__init__(self)
        self.cfg = cfg
        self.vocab = vocab
        self.enc = RNNEnc(vocab, cfg.emb, cfg.hid)
        self.dec = RNNDec(vocab, cfg.emb, cfg.hid * 2, cfg.hid)
